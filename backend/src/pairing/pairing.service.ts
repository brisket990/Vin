...
export class PairingService {
  ...
  /**
   * Like suggestForBottle but a full recipe idea rather than a short dish
   * name -- used for "apogée" (drinking window) alerts (see
   * ApogeeReminderScheduler) as well as an on-demand lookup from the
   * bottle detail screen. Also stateless/not persisted, for the same
   * reason as suggestForBottle.
   */
  async suggestRecipeForBottle(householdId: string, bottleId: string, provider?: string) {
    const [bottle] = await this.db
      .select()
      .from(bottles)
      .where(and(eq(bottles.id, bottleId), eq(bottles.householdId, householdId)))
    ...
    const { structured, rawResponse } = await client.suggestRecipeForBottle(candidate);
